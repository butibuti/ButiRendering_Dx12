...
public:
	RootSignature_Dx12(const std::int32_t arg_srvCount, const std::int32_t arg_cBufferCount, const std::vector< D3D12_GPU_DESCRIPTOR_HANDLE >& arg_samplerHandles, Value_weak_ptr<GraphicDevice_Dx12> arg_vwp_graphicDevice);
...
private:
	...
	std::int32_t m_textureRegion;
	std::int32_t m_cBufferCount;
	std::vector< D3D12_GPU_DESCRIPTOR_HANDLE >m_vec_samplerBufferDescriptorHandle;
	Value_weak_ptr<GraphicDevice_Dx12> m_vwp_graphicDevice;
};
class PipelineState_Dx12 :public IPiplineState {
public:
