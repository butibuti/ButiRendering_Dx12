...
public:
	...
	Value_ptr<IPipelineState> GetPipelineState(const std::string& arg_key);
	Value_ptr<IPipelineState> GetPipelineState(const DrawSettings& arg_drawSettings, Value_ptr<IRootSignature> arg_vlp_rootSignature, Value_weak_ptr<IResource_Shader> arg_vwp_shader);
private:
	std::unordered_map<std::string, Value_ptr<IPipelineState>> m_map_pipeLineState;
