...
ButiEngine::ButiRendering::HandleInformation ButiEngine::ButiRendering::DescriptorHeapManager::GetSamplerHandle(const std::uint32_t key)
{
	D3D12_GPU_DESCRIPTOR_HANDLE gpuHandle =	samplerDescriptorHeap->GetGPUDescriptorHandleForHeapStart();
	gpuHandle.ptr +=samplerDescriptorHandleIncrementSize * key;
	D3D12_CPU_DESCRIPTOR_HANDLE cpuHandle = samplerDescriptorHeap->GetCPUDescriptorHandleForHeapStart(); 
	cpuHandle.ptr +=key*samplerDescriptorHandleIncrementSize;
...
ButiEngine::ButiRendering::HandleInformation ButiEngine::ButiRendering::DescriptorHeapManager::CreateConstantBufferView(void* p_value, const bool isKeep, const std::int32_t size )
{
	std::uint32_t sizeAligned = (size + 0xff) & ~0xff, numRequired = sizeAligned / 0x100, top;
...
void ButiEngine::ButiRendering::DescriptorHeapManager::ConstantBufferUpdate(void* p_value, const std::uint32_t index, const std::int32_t size)
{
